...
use crate::types::{
    AlertThresholds, DataKey, DeviceReading, DeviceRegistration, MonitoringParameters,
...
impl PatientVitalsContract {
    pub fn record_vital_signs(
        ...
        let mut history: Vec<VitalReading> = env.storage().persistent().get(&key).unwrap_or(Vec::new(&env));
    ...
    pub fn submit_device_reading(
        ...
        if !env.storage().persistent().has(&device_key) {
            return Err(2); // Device not registered
        }
    ...
    pub fn trigger_vital_alert(
        ...
        let mut alerts: Vec<VitalAlert> = env.storage().persistent().get(&key).unwrap_or(Vec::new(&env));
    ...
    pub fn get_vital_trends(
        env: Env,
        patient_id: Address,
        vital_type: Symbol, // In a real system, you'd filter by this
        start_date: u64,
        end_date: u64,
    ) -> Result<Vec<VitalReading>, u32> {
        ...
        let history: Vec<VitalReading> = env.storage().persistent().get(&key).unwrap_or(Vec::new(&env));
    ...
    fn extract_vital_value(env: &Env, vitals: &VitalSigns, vital_type: &Symbol) -> Option<u32> {
        if vital_type == &Symbol::new(env, "heart_rate") { return vitals.heart_rate; }
        if vital_type == &Symbol::new(env, "bp_systolic") { return vitals.blood_pressure_systolic; }
        if vital_type == &Symbol::new(env, "bp_diastolic") { return vitals.blood_pressure_diastolic; }
        if vital_type == &Symbol::new(env, "temperature") { return vitals.temperature; }
        if vital_type == &Symbol::new(env, "respiratory") { return vitals.respiratory_rate; }
        if vital_type == &Symbol::new(env, "oxygen_sat") { return vitals.oxygen_saturation; }
        if vital_type == &Symbol::new(env, "blood_glucose") { return vitals.blood_glucose; }
        if vital_type == &Symbol::new(env, "weight") { return vitals.weight; }
